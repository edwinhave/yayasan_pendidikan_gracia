...
export default function Harga({ auth, payments }: PageProps & { payments: any[] }) {
    ...
    return (
        <AuthenticatedLayout
            user={auth.user}
            header={
                <h2 className="font-semibold text-xl text-gray-800 leading-tight">Pembayaran</h2>
            }
        >
                        ...
                        <div className="flex flex-1 flex-col">
                            <div className="@container/main flex flex-1 flex-col gap-2 py-4">
                                <div className="flex justify-end px-6">
                                    <Button id="download-report" asChild>
                                        <a href={route('pembayaran.report')} target="_blank">
                                            <Download className="mr-2 h-4 w-4" /> Laporan Belum
                                            Bayar
                                        </a>
                                    </Button>
                                </div>
                                <div className="flex flex-col">
                                    <DataTable data={payments} columns={getColumns} />
                                </div>
                            </div>
                        </div>
                    </div>
